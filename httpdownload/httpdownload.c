...
typedef int (*hardw_upgrade_callback)(const char* buffer, int length);
...
int write_to_local_file(const char* buffer, int len)
{
	int write_len = 0;
	// 文件没有打开则先打开
    if(!g_info->save_file)
    {
        g_info->save_file = fopen(g_info->save_path, "w");
        if(!g_info->save_file)
        {
	...
	write_len = fwrite(buffer, sizeof(char), len, g_info->save_file);
	return write_len;
}
...
/* 下载主函数 */
int http_download(char *url, char *save_path, hardw_upgrade_callback cb)
{
    ...
    //初始化结构体
    g_info = malloc(sizeof(http_t));
    if(!g_info)
    {
        lprintf(MSG_ERROR, "malloc failed\n");
        return -1;
    }
    memset(g_info, 0, sizeof(http_t));
    g_info->sock = -1;
    g_info->save_path = save_path;
	g_info->cb = cb;

    // 解析url
    if(-1 == parser_URL(url, g_info))
        goto failed;

    // 连接到server
    if(-1 == connect_server(g_info))
        goto failed;

    // 发送http请求报文
    if(-1 == send_request(g_info))
        goto failed;

    // 接收响应的头信息
    g_info->in = fdopen(g_info->sock, "r");
    if(!g_info->in)
    {
    ...
    // 解析头部
    if(-1 == parse_http_header(g_info))
        goto failed;

    switch(g_info->status_code)
    {
        case HTTP_OK:
            // 接收数据
            lprintf(MSG_DEBUG, "recv data now\n");
            g_info->start_recv_time = time(0);
            if(-1 == recv_response(g_info))
                goto failed;

            g_info->end_recv_time = time(0);
            lprintf(MSG_INFO, "recv %d bytes\n", g_info->recv_data_len);
            lprintf(MSG_INFO, "Average download speed: %.2fKB/s\n", 
                    calc_download_speed(g_info)/1000);
            break;
        case HTTP_REDIRECT:
            // 重启本函数
            lprintf(MSG_INFO, "redirect: %s\n", g_info->location);
            strncpy(tmp, g_info->location, URI_MAX_LEN - 1);
            clean_up(g_info);
            return http_download(tmp, save_path, g_info->cb);
...
(5)端口号访问测试
./a.out "http://uiotsh.ipc365.com:8080/download/7682_upgrade/download/PUWELL_PW542K12S-AGRW_V9.01.82_20201208-upgrade++.bin" PUWELL_PW542K12S-AGRW_V9.01.82_20201208-upgrade++.bin
****************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    http_download(argv[1], argv[2], write_to_local_file);
    return 0;
}
